...
let userID = 'f12e9b57-4136-46a4-8368-0caacc2f7a85';
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname) {
					...
					default:
						...
						// For any other path, reverse proxy to 'www.visa.com' and return the original response, caching it in the process
...
function getVLESSConfig(userID, hostName) {
	const wvlessws = `vless://${userID}@visa.com:8080?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2048#${hostName}`;
	const pvlesswstls = `vless://${userID}@visa.com:8443?encryption=none&security=tls&type=ws&host=${hostName}&sni=${hostName}&fp=random&path=%2F%3Fed%3D2048#${hostName}`;
  ...
  <p>必须设置自定义域名后才能使用Workers方式的TLS模式，否则，建议只使用vless+ws节点即可</p>
